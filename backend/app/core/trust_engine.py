...
def _identity_confidence(db: OrmSession, card_id: str) -> tuple[float, str | None]:
    log = crud.get_latest_auth_log(db, card_id)
    if log is None:
        return 100.0, None
...
def _site_risk(db: OrmSession) -> tuple[float, str | None]:
    check = crud.get_latest_risk_check(db)
    if check is None:
        return 0.0, None
...
def _sustained_low_duration(db: OrmSession, session_id: str, now: datetime) -> float:
...
def compute_trust_score(db: OrmSession, session: SessionModel) -> dict:
    now = utcnow()

    id_score, id_reason = _identity_confidence(db, session.card_id)
    site_score, site_reason = _site_risk(db)
    ...
    sustained_duration = _sustained_low_duration(db, session.session_id, now)
